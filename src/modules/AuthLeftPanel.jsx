...
/** Classic gable house — hero-style */
function HouseGable({ style, fill, opacity = 0.25 }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 72" aria-hidden style={{ position: "absolute", width: 52, height: 58, opacity, ...style }}>
      ...
      <rect x="22" y="38" width="20" height="22" rx="1" fill="rgba(255,255,255,0.28)" stroke={INK} strokeWidth="1.2" />
    </svg>
  );
}

/** Slightly wider / shorter block — matches hero variety */
function HouseBlock({ style, fill, opacity = 0.25 }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 72 64" aria-hidden style={{ position: "absolute", width: 58, height: 52, opacity, ...style }}>
      ...
      <rect x="26" y="34" width="20" height="20" rx="1" fill="rgba(255,255,255,0.22)" stroke={INK} strokeWidth="1.1" />
    </svg>
  );
}

/** Tall narrow tower — hero skyline feel */
function HouseTower({ style, fill, opacity = 0.25 }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 56 80" aria-hidden style={{ position: "absolute", width: 44, height: 62, opacity, ...style }}>
      ...
      <rect x="22" y="44" width="12" height="14" rx="1" fill="rgba(255,255,255,0.2)" stroke={INK} strokeWidth="1" />
    </svg>
  );
}
...
/**
 * Decorative layer: many small houses, 20–30% opacity, scattered across full panel.
 * Deterministic positions (stable SSR / hydration).
 */
const FLOAT = [
