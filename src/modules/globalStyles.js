export const G = `
  ...
  .home-hero-copy { width: 100%; max-width: 52rem; margin: 0 auto; text-align: center; display: flex; flex-direction: column; align-items: center; flex: 0 0 auto; justify-content: center; padding-top: clamp(12px, 3vh, 40px); }
  .home-hero-headline { margin: 0; text-wrap: balance; }
  .home-hero-headline-line { display: block; color: #fff; }
  .home-hero-headline-line--sans { font-family: 'Plus Jakarta Sans', 'Inter', system-ui, sans-serif; font-size: clamp(42px, 6vw, 68px); line-height: 1.03; letter-spacing: -0.038em; font-weight: 800; }
  .home-hero-headline-line--serif { font-family: 'Fraunces', Georgia, 'Times New Roman', serif; font-size: clamp(32px, 4.5vw, 52px); line-height: 1.1; letter-spacing: -0.024em; font-weight: 700; margin-top: 0.12em; font-style: normal; }
  ...
  .home-hero-illustration-svg { width: max(132vw, 1480px); max-width: none; height: auto; min-height: min(58vh, 520px); flex-shrink: 0; will-change: transform; display: block; }
  ...
  .home-band-split { background: linear-gradient(165deg, rgba(248,250,252,0.96) 0%, rgba(236,242,252,0.55) 42%, rgba(244,247,251,0.92) 100%); padding: clamp(44px, 5.5vw, 68px) var(--home-gutter-x); border-top: 1px solid rgba(226,232,240,0.38); border-bottom: 1px solid rgba(226,232,240,0.38); position: relative; }
  .home-hero-qs-dock { position: relative; z-index: 4; width: min(460px, calc(100vw - 28px)); display: flex; flex-direction: column; align-items: stretch; gap: 0; pointer-events: none; margin: clamp(14px, 2.2vw, 22px) auto 0; }
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  :root {
    ...
    --home-card-border: 1px solid rgba(255,255,255,0.55);
    ...
    /* Premium shell tokens (feed, dashboard, cards — home-specific vars unchanged) */
    --r-sm: 10px;
    --r-md: 12px;
    --r-lg: 16px;
    --r-xl: 20px;
    --shadow-elev-soft: 0 1px 2px rgba(15,23,42,0.035), 0 10px 36px rgba(15,23,42,0.045);
    --shadow-elev-hover: 0 4px 20px rgba(15,23,42,0.055), 0 14px 44px rgba(15,23,42,0.06);
    --premium-ink: rgba(15, 23, 42, 0.92);
  }
  .card { background: var(--white); border-radius: var(--r-lg); border: 1px solid rgba(226,232,240,0.72); box-shadow: var(--shadow-elev-soft); transition: box-shadow 0.32s ease, transform 0.32s ease, border-color 0.24s ease; }
  .card:hover { box-shadow: var(--shadow-elev-hover); transform: translateY(-1px); border-color: rgba(203,213,225,0.85); }
  .northing-listing-card-media {
    position: relative;
    width: 100%;
    aspect-ratio: 4 / 3;
    background: linear-gradient(135deg, #e8f5ee, #c2e8d4);
    display: flex;
    align-items: center;
    justify-content: center;
    overflow: hidden;
  }
  .northing-price-tabular { font-variant-numeric: tabular-nums; }
  .card-flat { background: var(--white); border-radius: var(--r-md); border: 1px solid rgba(226,232,240,0.65); box-shadow: 0 1px 2px rgba(15,23,42,0.02); }
  .inp { background: var(--white); border: 1px solid rgba(226,232,240,0.95); color: var(--text); border-radius: var(--r-md); padding: 12px 15px; font-size: 14px; width: 100%; outline: none; transition: border 0.2s, box-shadow 0.2s, background 0.2s; font-family: inherit; }
  .inp::placeholder { color: var(--muted); }
  .inp:focus { border-color: rgba(26,26,26,0.35); box-shadow: 0 0 0 3px rgba(26,26,26,0.08); }
  .inp:focus-visible { border-color: rgba(26,26,26,0.38); box-shadow: 0 0 0 3px rgba(15,23,42,0.1); outline: none; }
  ...
  .glass-footer {
    background: linear-gradient(165deg, rgba(22, 32, 52, 0.94) 0%, rgba(15, 23, 42, 0.98) 45%, rgba(10, 16, 30, 0.99) 100%) !important;
    backdrop-filter: blur(36px) saturate(185%);
    -webkit-backdrop-filter: blur(36px) saturate(185%);
    border-top: 1px solid rgba(255,255,255,0.14);
    box-shadow: 0 -12px 48px rgba(0,0,0,0.28), inset 0 1px 0 rgba(255,255,255,0.08);
  }
  .home-footer { position: relative; overflow: hidden; padding: clamp(18px, 3vw, 32px) 0 max(14px, env(safe-area-inset-bottom)); }
  .home-footer-scrim { position: absolute; inset: 0; z-index: 1; pointer-events: none; background: radial-gradient(ellipse 120% 80% at 50% -20%, rgba(99, 102, 241, 0.12) 0%, transparent 55%), linear-gradient(180deg, rgba(15,23,42,0.35) 0%, rgba(8, 12, 24, 0.88) 45%, rgba(15,23,42, 0.97) 100%); }
  .home-footer-inner { max-width: var(--home-content-max); margin: 0 auto; position: relative; z-index: 2; padding: 0 var(--home-gutter-x); box-sizing: border-box; display: flex; flex-direction: column; align-items: stretch; gap: 0; width: 100%; }
  .home-footer-top { display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 1.35fr); gap: clamp(14px, 2.5vw, 28px); align-items: start; width: 100%; }
  .home-footer-brand { display: flex; flex-direction: column; gap: 6px; min-width: 0; }
  .home-footer-logo-btn { padding: 0; border: none; background: none; cursor: pointer; align-self: flex-start; -webkit-tap-highlight-color: transparent; }
  .home-footer-logo { height: 38px; width: auto; max-width: min(240px, 100%); display: block; object-fit: contain; }
  .home-footer-tagline { margin: 0; font-size: 12px; line-height: 1.45; color: rgba(248,250,252,0.58); font-weight: 500; letter-spacing: -0.012em; max-width: 26em; }
  .home-footer-nav { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 8px; width: 100%; min-width: 0; }
  @media (min-width: 520px) {
    .home-footer-nav { grid-template-columns: repeat(3, minmax(0, 1fr)); }
  }
  @media (min-width: 900px) {
    .home-footer-nav { grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 8px; }
  }
  .home-footer-link {
    font-family: inherit; font-size: 12px; font-weight: 600; letter-spacing: -0.012em; text-align: center;
    padding: 9px 11px; border-radius: 11px; margin: 0;
    border: 1px solid rgba(255,255,255,0.14);
    background: linear-gradient(165deg, rgba(255,255,255,0.1) 0%, rgba(255,255,255,0.04) 100%);
    backdrop-filter: blur(16px) saturate(170%);
    -webkit-backdrop-filter: blur(16px) saturate(170%);
    color: rgba(248,250,252,0.92);
    box-shadow: 0 1px 0 rgba(255,255,255,0.08) inset, 0 6px 20px rgba(0,0,0,0.12);
    cursor: pointer; -webkit-tap-highlight-color: transparent; touch-action: manipulation;
    transition: transform 0.22s ease, box-shadow 0.28s ease, border-color 0.22s ease, background 0.22s ease, color 0.2s ease;
  ...
  .home-footer-social-a {
    ...
    text-decoration: none; transition: transform 0.2s ease, box-shadow 0.22s ease, border-color 0.2s ease, background 0.2s ease, color 0.2s ease;
    -webkit-tap-highlight-color: transparent;
  }
  .home-footer-social-a:hover { background: linear-gradient(165deg, rgba(255,255,255,0.2) 0%, rgba(255,255,255,0.08) 100%); border-color: rgba(255,255,255,0.28); color: #fff; transform: translateY(-1px); box-shadow: 0 1px 0 rgba(255,255,255,0.12) inset, 0 8px 22px rgba(0,0,0,0.15); }
  .home-footer-social-a--disabled { opacity: 0.4; cursor: default; pointer-events: none; }
  .home-footer-social-a--disabled:hover { transform: none; box-shadow: 0 1px 0 rgba(255,255,255,0.06) inset; border-color: rgba(255,255,255,0.14); background: linear-gradient(165deg, rgba(255,255,255,0.11) 0%, rgba(255,255,255,0.04) 100%); color: rgba(248,250,252,0.95); }
  .home-footer-divider { height: 1px; width: 100%; margin: clamp(12px, 2vw, 16px) 0; background: linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.18) 50%, transparent 100%); opacity: 0.85; }
  .home-footer-bottom { display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; gap: 10px 18px; width: 100%; }
  .home-footer-copy { margin: 0; font-size: 11px; line-height: 1.5; color: rgba(248,250,252,0.45); font-weight: 500; max-width: 48em; }
  .home-footer-legal { display: flex; align-items: center; gap: 4px; flex-wrap: wrap; }
  .home-footer-legal-link { background: rgba(255,255,255,0.04); border: 1px solid transparent; padding: 5px 11px; border-radius: 8px; font: inherit; font-size: 11px; font-weight: 600; color: rgba(248,250,252,0.62); cursor: pointer; transition: color 0.2s, background 0.2s, border-color 0.2s; -webkit-tap-highlight-color: transparent; touch-action: manipulation; }
  .home-footer-legal-link:hover { color: #fff; background: rgba(255,255,255,0.1); border-color: rgba(255,255,255,0.12); }
  @media (prefers-reduced-motion: reduce) {
    .home-footer-link { transition: border-color 0.2s, background 0.2s, color 0.2s; }
    .home-footer-link:hover { transform: none; }
  }
  .glass-nav-enhance {
  ...
  .glass-nav-enhance.northing-nav--scrolled {
  ...
  .home-hero-section--light .home-hero-inner { justify-content: center; padding-bottom: max(44px, 5vh); }
  .home-hero-tagline { margin: 0.85rem 0 0; max-width: min(36rem, 100%); font-size: clamp(14px, 1.85vw, 16px); line-height: 1.5; font-weight: 500; letter-spacing: -0.012em; text-align: center; color: #64748b; }
  .home-hero-section--light .home-hero-tagline { color: #64748b; }
  ...
  .home-section-subnav { position: sticky; top: 72px; z-index: 90; background: rgba(255,255,255,0.72); backdrop-filter: blur(22px) saturate(200%); -webkit-backdrop-filter: blur(22px) saturate(200%); border-bottom: 1px solid rgba(255,255,255,0.45); box-shadow: 0 1px 0 rgba(15,23,42,0.04), inset 0 1px 0 rgba(255,255,255,0.85); }
  ...
  .home-page-shell { background: linear-gradient(180deg, #ffffff 0%, #f8fafc 42%, #f1f5f9 100%); width: 100%; max-width: 100%; box-sizing: border-box; }
  .home-prop-section { padding: clamp(36px, 4.5vw, 56px) var(--home-gutter-x) clamp(44px, 5.5vw, 64px); max-width: var(--home-content-max); margin: 0 auto; width: 100%; box-sizing: border-box; background: linear-gradient(180deg, rgba(255,255,255,0.97) 0%, rgba(248,250,252,0.88) 100%); }
  ...
  .home-sample-pane { --sample-cream: #f9f6f0; --sample-card: #ede8e0; --sample-brown: #3c2a2c; --sample-brown-mid: #5c4548; position: relative; background: linear-gradient(180deg, rgba(252,250,246,0.98) 0%, rgba(245,240,232,0.85) 45%, rgba(249,246,240,0.95) 100%); padding: clamp(40px, 5.5vw, 72px) 0; border: none; overflow-x: clip; }
  ...
  @media (max-width: 900px) {
    .home-sample-pane-grid-outer { margin-top: 20px; padding-left: max(16px, env(safe-area-inset-left)); padding-right: 0; overflow-x: auto; -webkit-overflow-scrolling: touch; overscroll-behavior-x: contain; scroll-snap-type: x mandatory; scroll-padding-inline: max(16px, env(safe-area-inset-left)); scrollbar-width: thin; }
    .home-sample-pane-grid { display: flex; flex-direction: row; flex-wrap: nowrap; gap: 14px; max-width: none; width: max-content; min-width: 100%; align-items: stretch; margin-left: 0; margin-right: 0; }
    .home-sample-pane-card-wrap { flex: 0 0 min(86vw, 340px); width: min(86vw, 340px); max-width: min(86vw, 340px); scroll-snap-align: start; }
  }
  ...
  .home-band-card { padding: 30px 28px; display: block; border-radius: var(--home-card-radius); }
  ...
  .home-band-cta { position: relative; background: linear-gradient(175deg, rgba(241,245,249,0.75) 0%, rgba(248,250,252,0.95) 38%, rgba(238,242,248,0.65) 100%); padding: clamp(44px, 5.5vw, 68px) var(--home-gutter-x); overflow: hidden; }
  ...
  .home-page-shell .card.glass-card { border-radius: var(--home-card-radius); background: linear-gradient(155deg, rgba(255,255,255,0.88) 0%, rgba(255,255,255,0.58) 100%); border: 1px solid rgba(255,255,255,0.65); box-shadow: 0 4px 24px rgba(15,23,42,0.06), 0 1px 2px rgba(15,23,42,0.03), inset 0 1px 0 rgba(255,255,255,0.95), inset 0 0 0 1px rgba(255,255,255,0.4); backdrop-filter: blur(28px) saturate(200%); -webkit-backdrop-filter: blur(28px) saturate(200%); transition: box-shadow 0.4s ease, transform 0.4s ease, border-color 0.28s ease; }
  .home-page-shell .card.glass-card:hover { transform: translateY(-1px); border-color: rgba(255,255,255,0.78); box-shadow: 0 8px 36px rgba(15,23,42,0.08), 0 2px 8px rgba(15,23,42,0.04), inset 0 1px 0 rgba(255,255,255,1), inset 0 0 0 1px rgba(255,255,255,0.55); }
  .home-animate-on-scroll { opacity: 0; transform: translateY(22px); transition: opacity 0.65s cubic-bezier(0.22, 1, 0.36, 1), transform 0.65s cubic-bezier(0.22, 1, 0.36, 1); will-change: opacity, transform; }
  .home-animate-on-scroll.home-animate-in { opacity: 1; transform: translateY(0); will-change: auto; }
  @media (prefers-reduced-motion: reduce) {
    .home-animate-on-scroll { opacity: 1; transform: none; transition: none; will-change: auto; }
  }
  ...
  @media(max-width:768px){
    .h1big-hero.home-hero-section { isolation: isolate !important; align-items: stretch !important; min-height: clamp(320px, 48dvh, 520px) !important; }
    ...
    .h1big-hero.home-hero-section .home-hero-copy { position: relative !important; z-index: 2 !important; flex: 0 0 auto !important; justify-content: flex-start !important; align-items: center !important; max-width: min(100%, 380px) !important; width: 100% !important; margin-left: auto !important; margin-right: auto !important; padding: 14px 0 11px !important; gap: 0 !important; box-sizing: border-box !important; }
    ...
    .h1big-hero.home-hero-section .home-hero-inner .home-hero-qs-dock { position: relative !important; left: auto !important; right: auto !important; bottom: auto !important; top: auto !important; width: 100% !important; max-width: min(380px, 100%) !important; margin: 12px auto 0 !important; z-index: 5 !important; pointer-events: none !important; align-self: center !important; flex-shrink: 0 !important; box-sizing: border-box !important; }
    ...
    .h1big-hero.home-hero-section .home-hero-parallax-bg--illustration .home-hero-illustration-svg { width: min(150vw, 900px) !important; min-height: 0 !important; max-height: min(20vh, 180px) !important; opacity: 0.62 !important; transform: none !important; filter: saturate(0.82) !important; }
    .h1big-hero.home-hero-section .home-hero-illustration-svg--heroMobile { max-height: min(20vh, 180px) !important; opacity: 0.62 !important; }
    ...
    .home-page-shell{overflow-x:clip!important;max-width:100%!important;padding-bottom:max(20px, env(safe-area-inset-bottom))!important}
    ...
    .home-hero-section--light .home-hero-inner{justify-content:center!important;padding-bottom:max(20px, env(safe-area-inset-bottom))!important}
    ...
    .home-hero-headline-line--sans{font-size:clamp(32px,8.2vw,46px)!important;line-height:1.04!important;letter-spacing:-0.035em!important}
    .home-hero-headline-line--serif{font-size:clamp(25px,6.2vw,34px)!important;line-height:1.08!important;margin-top:0.1em!important}
    .home-hero-tagline{margin-top:10px!important;margin-bottom:0!important;padding:0 6px!important;font-size:14px!important;line-height:1.5!important;color:#64748b!important;max-width:min(100%,380px)!important}
    ...
    .home-prop-section{padding:22px max(20px, env(safe-area-inset-left)) 28px max(20px, env(safe-area-inset-right))!important;max-width:100%!important;width:100%!important;box-sizing:border-box!important}
    ...
    .home-prop-section .gr-listings,.home-prop-section .gr.gr-listings{display:flex!important;flex-direction:row!important;flex-wrap:nowrap!important;align-items:stretch!important;gap:14px!important;width:100%!important;max-width:100%!important;overflow-x:auto!important;-webkit-overflow-scrolling:touch!important;overscroll-behavior-x:contain!important;scroll-snap-type:x mandatory!important;scroll-padding-inline:max(20px, env(safe-area-inset-left))!important;padding-bottom:6px!important;scrollbar-width:thin!important}
    .home-prop-section .gr-listings::-webkit-scrollbar{height:5px!important}
    .home-prop-section .gr-listings::-webkit-scrollbar-thumb{background:rgba(148,163,184,0.55)!important;border-radius:999px!important}
    .home-prop-section .home-listing-card,.home-prop-section .gr-listings>div{flex:0 0 min(86vw, 340px)!important;width:min(86vw, 340px)!important;max-width:min(86vw, 340px)!important;min-width:0!important;scroll-snap-align:start!important;box-sizing:border-box!important;touch-action:manipulation!important;-webkit-tap-highlight-color:rgba(0,0,0,0)!important}
    ...
    .home-sample-pane-grid-outer{margin-top:20px!important;padding-left:max(16px, env(safe-area-inset-left))!important;padding-right:0!important;overflow-x:auto!important;-webkit-overflow-scrolling:touch!important;overscroll-behavior-x:contain!important;scroll-snap-type:x mandatory!important;scroll-padding-inline:max(16px, env(safe-area-inset-left))!important;scrollbar-width:thin!important}
    .home-sample-pane-grid-outer::-webkit-scrollbar{height:5px!important}
    .home-sample-pane-grid-outer::-webkit-scrollbar-thumb{background:rgba(148,163,184,0.45)!important;border-radius:999px!important}
    .home-sample-pane-grid{display:flex!important;flex-direction:row!important;flex-wrap:nowrap!important;gap:14px!important;max-width:none!important;width:max-content!important;min-width:100%!important;align-items:stretch!important;margin-left:0!important;margin-right:0!important}
    .home-sample-pane-card-wrap{flex:0 0 min(86vw, 340px)!important;width:min(86vw, 340px)!important;max-width:min(86vw, 340px)!important;scroll-snap-align:start!important}
    ...
    .home-wa-sample-frame{width:100%!important;height:auto!important;max-height:240px!important;aspect-ratio:1/1!important;max-width:min(280px,100%)!important;margin-left:auto!important;margin-right:auto!important}
    .home-sample-pane .home-sample-wa-mock{max-height:200px!important}
    .home-sample-pane .home-sample-pdf-mock{max-height:200px!important;aspect-ratio:3/4!important}
    .home-band-split-inner{grid-template-columns:1fr!important;gap:14px!important}
    .home-band-card{padding:22px 18px!important}
    ...
    .home-cta-grid{display:flex!important;flex-direction:row!important;flex-wrap:nowrap!important;gap:14px!important;overflow-x:auto!important;-webkit-overflow-scrolling:touch!important;overscroll-behavior-x:contain!important;scroll-snap-type:x mandatory!important;scroll-padding-inline:max(20px, env(safe-area-inset-left))!important;padding-bottom:8px!important;scrollbar-width:thin!important}
    .home-cta-grid::-webkit-scrollbar{height:4px!important}
    .home-cta-grid::-webkit-scrollbar-thumb{background:rgba(148,163,184,0.45)!important;border-radius:999px!important}
    .home-cta-grid .card.glass-card{flex:0 0 min(88vw, 340px)!important;max-width:min(88vw, 340px)!important;scroll-snap-align:start!important}
    ...
    .feed-page{overflow-x:clip!important;box-sizing:border-box!important;padding-bottom:max(24px, env(safe-area-inset-bottom))!important}
    ...
    .feed-page-inner{padding:0 max(20px, env(safe-area-inset-left)) 32px max(20px, env(safe-area-inset-right))!important;margin-top:-40px!important;box-sizing:border-box!important;max-width:100%!important}
    ...
    .home-footer{padding-bottom:max(16px, env(safe-area-inset-bottom))!important}
    .home-footer-inner{padding:0 max(16px, env(safe-area-inset-left)) max(2px, env(safe-area-inset-bottom)) max(16px, env(safe-area-inset-right))!important}
    .home-footer-top{grid-template-columns:1fr!important;gap:16px!important}
    .home-footer-brand{align-items:center!important;text-align:center!important}
    .home-footer-logo-btn{align-self:center!important}
    ...
    .home-footer-legal{justify-content:center!important}
    .home-footer-link{min-height:44px!important}
    .home-footer-legal-link{min-height:40px!important;padding:8px 14px!important;font-size:11px!important}
  ...
  @media(max-width:480px){
    ...
    .home-sample-pane-grid{max-width:none!important;width:max-content!important;min-width:100%!important}
  }
  ...
  .property-detail-breadcrumb__link {
    background: none;
    border: none;
    padding: 0;
    font: inherit;
  ...
  .property-detail-breadcrumb__link:hover { color: #1a1a1a; }
  .property-detail-breadcrumb__sep { color: #cbd5e1; user-select: none; }
  .property-detail-breadcrumb__current {
    flex: 1 1 140px;
    min-width: 0;
    color: #1a1a1a;
    font-weight: 600;
  ...
  .property-detail-price { font-family: 'Fraunces', Georgia, serif; font-size: clamp(1.65rem, 5vw, 2rem); font-weight: 700; color: #1a1a1a; margin: 0 0 6px; letter-spacing: -0.02em; font-variant-numeric: tabular-nums; }
  ...
  .property-detail-video-panel {
    margin: 0 0 28px;
    max-width: 720px;
  }
  .property-detail-video-actions {
    margin-top: 18px;
  ...
  .property-detail-video-actions__eyebrow {
    margin: 0 0 6px;
    font-size: 10px;
    font-weight: 800;
  ...
  .property-detail-video-actions__sub {
    margin: 0 0 16px;
    font-size: 13px;
    line-height: 1.5;
    color: #475569;
  ...
  .property-detail-video-actions__row {
    display: flex;
    flex-wrap: wrap;
    gap: 10px;
    align-items: stretch;
  }
  .property-detail-video-actions__btn {
    flex: 1 1 160px;
    min-height: 48px;
    padding: 0 16px;
    border-radius: 12px;
  ...
  .property-detail-video-actions__btn:active:not(:disabled) { transform: scale(0.98); }
  .property-detail-video-actions__btn:disabled { opacity: 0.5; cursor: not-allowed; }
  .property-detail-video-actions__btn-icon { display: flex; color: #0f172a; opacity: 0.88; }
  .property-detail-video-actions__btn-icon--light { color: #fff; opacity: 1; }
  .property-detail-video-actions__btn--secondary {
    background: #fff;
    color: #0f172a;
    border: 1px solid rgba(15, 23, 42, 0.12);
    box-shadow: 0 1px 2px rgba(15, 23, 42, 0.05);
  }
  .property-detail-video-actions__btn--secondary:hover:not(:disabled) {
    border-color: rgba(15, 23, 42, 0.22);
    box-shadow: 0 4px 16px rgba(15, 23, 42, 0.08);
  }
  .property-detail-video-actions__btn--wa {
  ...
  @media (prefers-reduced-motion: reduce) {
    ...
    .btn-green:hover,
    .btn-green:active { transform: none !important; }
    .btn-outline:hover,
    .btn-outline:active { transform: none !important; }
    .btn-ghost:hover,
    .btn-ghost:active { transform: none !important; }
    .nav-mob-toggle:active { transform: none !important; }
    .property-detail-back:active { transform: none !important; }
  }
